<div class="modal fade" id="edit-product-rate" tabindex="-1" role="dialog" aria-hidden="true"
    ...
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            ...
            <div class="modal-body" ng-if="currentModal === '#edit-product-rate'">
                @include('admin.product_rates.form')
            </div>
            <div class="modal-footer">
                <button type="button" class="btn btn-danger btn-cons" ng-click="submit(0)" ng-disabled="loading.submit" ng-if="form.status == 1 ">
                    <i ng-if="!loading.submit" class="fa fa-times"></i>
                    <i ng-if="loading.submit" class="fa fa-spin fa-spinner"></i>
                    Không duyệt
                </button>
                <button type="button" class="btn btn-success btn-cons" ng-click="submit(2)" ng-disabled="loading.submit" ng-if="form.status == 1">
...
<script>
    app.controller('EditProductRate', function ($rootScope, $scope, $http, $timeout) {
        $rootScope.$on("editProductRate", function (event, data){
            $scope.form = new ProductRate(data, {scope: $scope});
            $scope.$applyAsync();
            ...
            $scope.currentModal = '#edit-product-rate';
